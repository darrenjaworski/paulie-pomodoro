...
export class PomodoroWebviewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'pauli-pomodoro-webview'; // Matches the id in package.json

    private _view?: vscode.WebviewView;

    constructor(private readonly _extensionUri: vscode.Uri) { }

    public updateTimerDisplay(minutes: number, seconds: number, sessionType: 'work' | 'break', isActive: boolean) {
        if (this._view) {
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(data => {
            switch (data.type) {
                case 'startTimer':
                    vscode.commands.executeCommand('pauli-pomodoro.startTimer');
                    break;
                case 'pauseTimer':
                    vscode.commands.executeCommand('pauli-pomodoro.pauseTimer');
                    break;
                case 'resetTimer':
                    vscode.commands.executeCommand('pauli-pomodoro.resetTimer');
                    break;
                case 'switchSession':
                    vscode.commands.executeCommand('pauli-pomodoro.switchSessionType');
                    break;
            }
        });
    ...
    private _getHtmlForWebview(webview: vscode.Webview): string {
        ...
        return `<!DOCTYPE html>
            ...
            <head>
                ...
                <title>Pauli Pomodoro</title>
            </head>
            <body>
                <div class="container">
                    <h1>Pauli Pomodoro</h1>
